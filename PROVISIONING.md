# How to provision a new environment for graff-tracker
...
* Enter domains to secure (for example): `graff-tracker.herokuapp.com`
* Enter webroot path: `/home/deployer/apps/graff_tracker/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/graff-tracker.herokuapp.com/fullchain.pem /etc/ssl/graff_tracker.crt
ln -s /etc/letsencrypt/live/graff-tracker.herokuapp.com/privkey.pem /etc/ssl/graff_tracker.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/graff_tracker/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
28 2,14 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/graff_tracker/shared/public/
```
